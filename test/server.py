...
def test_utf_request_path(server):
    import test_server
    print(test_server.__file__)
    #res = urlopen((server.get_url() + u'фыва').encode('utf-8'))
    data = send_get_request(server.address, server.port, u'/фыва'.encode('utf-8'))
    print(data)
    assert server.request['path'] == u'фыва'
